...
def main(args: argparse.Namespace) -> None:
    ...
    print(
        "INFO: The probability of any individual read being selected is: {0} (raw), {1} (multiplier-adjusted)".format(
    ...
    if output_reads_written < args.output_read_count:
        raise Exception(
            "ERROR: Unfortunately only {0} of the requested {1} reads were randomly selected.  "
...
def check_input_options(args):
    ...
    if args.single is not None:
        ...
        if len(left_files) != len(right_files):
            raise Exception(
                "ERROR: Count of left files ({0}) must be the same as right files ({1})".format(
